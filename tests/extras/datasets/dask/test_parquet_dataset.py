...
class TestParquetDataSet:
    ...
    @pytest.mark.parametrize(
        "save_args",
        [{"schema": {"col1": "list_(list_(int64()))", "col2": "string()"}}],
        indirect=True,
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            {
                "schema": "schema(\
                [field('c1', list_(int64())), field('c2', int64())])"
            }
        ],
        indirect=True,
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            {
                "schema": "schema(\
                [('c1', list_(int64())), ('c2', int64())])"
            }
        ],
        indirect=True,
    )
    def test_save_extra_params_schema_str_schema_tuple(self, s3_data_set, save_args):
        """Test setting the schema as string pyarrow schema (tuple of name and data type)
        in save arguments."""
